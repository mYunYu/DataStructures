...
class HashTable{
    ...
    public void findEmpById(int id){
        ...
        if(emp != null){            //找到雇员信息
            System.out.printf("在第%d条链表中找到， 雇员id = %d， 雇员name = %s\n", (empLinkedListNo + 1), id, emp.name);
        }
        else{
...
class EmpLinkedList{
    ...
    public Emp findEmpById(int id){
        //判断链表是否为空
        if(head == null){
            System.out.println("链表为空，找不到需要查找的雇员");
            return null;
        }
        //辅助指针
        Emp curEmp = head;
        while (true){
            if(curEmp.id == id){        //说明找到,这是curEmp就指向要查找的雇员
                break;
            }
    ...
    public void deleteEmpById(int id){
        ...
        while (true){
            ...
            //退出
            if(curEmp.next == null){        //说明已经是链表最后
                break;
            }

            if (curEmp.next.id == id) {      //说明找到，这是curEmp，就指向要删除的雇员
//                flag = true;
